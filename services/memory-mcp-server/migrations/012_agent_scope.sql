-- Migration 012: agent_scope text[] — per-agent visibility array
-- Adds an agent_scope array column to memories that lists which agents can see a memory.
-- Default ARRAY['shared'] means visible to all agents (backward compatible).
-- Examples:
--   ARRAY['shared']          → visible to all agents (default)
--   ARRAY['wren','iris']     → visible only to Wren and Iris
--   ARRAY['wren']            → Wren-private (preferred over visibility=private for new work)
--
-- This supersedes the binary visibility/agent_id approach with finer-grained control.
-- Apply via: Supabase Dashboard -> SQL Editor -> paste and run
-- https://supabase.com/dashboard/project/ogqjjlbupqnvlcyrfnxi/sql/new

-- ─── Step 1: Add agent_scope column ──────────────────────────────────────────

ALTER TABLE memories
  ADD COLUMN IF NOT EXISTS agent_scope text[] DEFAULT ARRAY['shared']::text[];

CREATE INDEX IF NOT EXISTS idx_memories_agent_scope ON memories USING GIN(agent_scope);

-- Backfill: existing shared memories → ARRAY['shared']
-- Existing private memories → ARRAY[agent_id] (preserve owner)
UPDATE memories
SET agent_scope = CASE
  WHEN visibility = 'private' AND agent_id IS NOT NULL THEN ARRAY[agent_id]::text[]
  ELSE ARRAY['shared']::text[]
END
WHERE agent_scope IS NULL OR agent_scope = ARRAY['shared']::text[];

-- ─── Step 2: Update hybrid_recall to support p_agent_scope ───────────────────
-- Adds optional p_agent_scope text param. When set:
--   'shared' = ANY(m.agent_scope) OR p_agent_scope = ANY(m.agent_scope)
-- When NULL, no scope filtering (backward compat).
...
GRANT EXECUTE ON FUNCTION public.apply_agent_scope_if_missing() TO service_role, authenticated;
