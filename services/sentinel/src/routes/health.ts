...
export function healthRouter(poller: Poller, guardian?: GuardianAgent): Router {
  ...
  router.post('/extension/heartbeat', (req: Request, res: Response) => {
  ...
  router.get('/extension/status', (_req: Request, res: Response) => {
  ...
  router.post('/extension/reconnect-done', (_req: Request, res: Response) => {
    if (!guardian) return res.status(503).json({ error: 'Guardian not enabled' });
