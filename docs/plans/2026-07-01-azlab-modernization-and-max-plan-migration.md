...
- **Harden Lumen's Supabase path:** route Lumen's *reads* through the Agent Bus (like writes already are), so the extension needs no Supabase key at all and the bus is the single Supabase gateway. Kills the recurring anon-key 401s (twice now: 2026-05-28 RLS denial, 2026-07-01 disabled legacy key) and de-risks the Strategy-5 RLS hardening, which would otherwise break the direct anon reads. Add a `/reads/<table>` path to the bus + point `supabaseRead` at it.
